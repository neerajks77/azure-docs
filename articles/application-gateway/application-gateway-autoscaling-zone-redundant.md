...
- **Static VIP**: Application gateway v2 SKU supports the static VIP type exclusively. This ensures that the VIP associated with application gateway doesn't change for the lifecycle of the deployment, even after a restart. This was not the case with V1, where we had the VIP, but it was not static. For a subdomain to route correctly to an App Service via Application Gateway, we had to use the Gateway URL for the CNAME record in the DNS instead of the IP address in V1.
